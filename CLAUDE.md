...
## Repo facts
- App: FastAPI backend + vanilla-JS frosted-glass UI; downloads via the yt-dlp
  CLI (one process per video), channel listing via scrapetube (yt-dlp fallback).
- yt-dlp's JavaScript challenges (signature / n-sig) are solved with the Deno
  runtime (copied into the image) + the `yt-dlp-ejs` solver scripts.
- Image is published to GHCR by `.github/workflows/docker-publish.yml`; the
  source repo is private but the image/package is public.
- Persistent state lives in `/config` (jobs, cookies, download archive,
  `failed_downloads.txt`). Downloads default to `/downloads`.
